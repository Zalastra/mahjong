...
impl TileTextures {
    ...
    pub fn load_textures(&mut self, renderer: &Renderer) {
        ...
        let side_texture = renderer.load_texture(Path::new("img/TileSide.png")).expect(ERROR_MESSAGE);
        let bottom_texture = renderer.load_texture(Path::new("img/TileBottom.png")).expect(ERROR_MESSAGE);
